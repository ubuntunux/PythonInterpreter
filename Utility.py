...
def loading_postprocess():
    try:
        global gMyRoot, gDebug, log, Print, gPyInterpreter
        ...
        gPyInterpreter = PyInterpreter.PyInterpreter.instance()
    except:
        print(traceback.format_exc())
...
class DebugPrint(Widget, Singleton):
    ...
    def init(self, bShow):
        ...
        # label
        self.debugLabel = TextInput(text="", halign='left', valign='top', readonly=True, use_bubble=False,
                                    use_handles=False,
                                    allow_copy=False, multiline=True, background_color=(1, 1, 1, 0),
...
class MyRoot(App, Singleton):
    ...
    def createProgressPopup(self, title, itemCount):
        ...
        self.progress = ProgressBar(value=0, max=itemCount, pos=sub(cXY, (pbSize[0] * 0.5, sizehintH * H * 0.75)),
                                    size=pbSize)
